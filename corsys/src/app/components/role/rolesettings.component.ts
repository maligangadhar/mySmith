import { Component, OnDestroy, Input, ViewChild, TemplateRef, Inject } from '@angular/core';
import { sortOrder, opMode, responseStatus, messageType } from '../../models/enums';
import { IApp, IKeyData, IResponse, IKeyValue, IRoleDetail } from '../../models/viewModels';
import { ModalCreateRoleComponent } from '../modal/modal.role.create.component';
import { Subject } from 'rxjs/Subject';
import { ICommonService, IRoleService, IMessageService, IBroadcastService, ISortService } from '../../interfaces/interfaces';

@Component({
  selector: 'sp3-comp-app-role-settings',
  templateUrl: './rolesettings.component.html'
})
export class RoleSettingsComponent implements OnDestroy {
  ...
  showaddrolepanel:(event) => void;
  showAddNewRolePanel:boolean=false;
  ...
  constructor( @Inject('ICommonService') private commonService: ICommonService,
   ...
   /*   vm.handleCardUpdated = (flag) => {
      ...
      vm.showaddrolepanel=(event) =>{
        this.showAddNewRolePanel=event;
      };
